...
DEFAULT_DOCS = {
    'Proprietorship': [
        'Aadhaar of Proprietor', 'PAN of Proprietor', 'GST Certificate', 'Shop/Establishment License',
        'Business Address Proof', 'Bank Statement (12 months)', 'ITR (2-3 years)'
    ],
    'Partnership': [
        'Partnership Deed', "PAN of Firm", 'GST Certificate', 'Address Proof of Firm',
        "Partners' Aadhaar & PAN", 'Bank Statement (12 months)', 'ITR (2-3 years)'
    ],
    'LLP': [
        'LLP Agreement', 'PAN of LLP', 'Certificate of Incorporation', 'GST Certificate',
        'Address Proof of LLP', "Partners' Aadhaar & PAN", 'Bank Statement (12 months)', 'ITR (2-3 years)'
    ],
    'Pvt Ltd': [
        'MOA & AOA', 'Certificate of Incorporation', 'Company PAN', 'GST Certificate',
        "Directors' Aadhaar & PAN", 'Board Resolution for Loan', 'Bank Statement (12 months)', 'ITR (2-3 years)'
    ],
    'Salaried': [
...
def add_client():
    ...
    # attach default documents for this type
        ...
        for d in docs:
            cd = ClientDocument(client_id=client.id, document_id=d.id, received=False)
            db.session.add(cd)
        db.session.commit()
...
def client_detail(client_id):
    ...
    docs = sorted(client.documents, key=lambda x: x.document.name.lower())
...
def edit_documents(client_id):
    ...
    if request.method == 'POST':
        ...
        # Create a Document entry under the client's type (this will become available for future clients of same type)
            doc = Document.query.filter_by(client_type=client.client_type, name=new_doc_name).first()
            if not doc:
                doc = Document(client_type=client.client_type, name=new_doc_name)
                db.session.add(doc)
                db.session.commit()
            # attach to the client
            cd = ClientDocument(client_id=client.id, document_id=doc.id, received=False)
            db.session.add(cd)
            db.session.commit()
